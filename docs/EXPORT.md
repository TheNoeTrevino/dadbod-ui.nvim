...
- [`lua/dadbod-ui/export/init.lua`](../lua/dadbod-ui/export/init.lua)
  orchestrates. Reads the query + connection off the `.dbout` buffer, re-runs it
  through the adapter CLI with `vim.system`, writes the file. It does NOT go
  through dadbod's `:DB` job, so it never collides with a running query. The
