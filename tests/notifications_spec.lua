...
describe('notifications', function()
  ...
  it('routes info/warn/error through vim.notify with the [Dadbod-UI] title', function()
    state.setup({})
    ...
    assert.equals('[Dadbod-UI]', notify_calls[1].opts.title)
  ...
  it('suppresses info when disable_info_notifications is set', function()
    state.setup({ notifications = { disable_info = true } })
  ...
  it('uses the echo backend when force_echo_notifications is set', function()
    state.setup({ notifications = { force_echo = true } })
    notifications.error('boom')

    assert.equals(0, #notify_calls)
    assert.equals(1, #echo_calls)
    assert.equals('[Dadbod-UI] boom', echo_calls[1].chunks[1][1])
  ...
  it('lets a per-call echo opt force the echo backend', function()
    ...
    assert.equals(0, #notify_calls)
    assert.equals(1, #echo_calls)
    assert.equals('[Dadbod-UI] cmdline', echo_calls[1].chunks[1][1])
  end)
  ...
  it('replaces info toasts under use_nvim_notify', function()
    state.setup({ notifications = { use_nvim_notify = true } })
