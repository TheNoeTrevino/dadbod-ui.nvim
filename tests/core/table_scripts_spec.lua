...
describe('table_scripts: capability presence + action set', function()
  it('postgres exposes table_scripts; unported adapters do not', function()
    ...
    assert.same({ 'CREATE To', 'DROP To', 'SELECT To', 'INSERT To', 'UPDATE To', 'DELETE To' }, labels)
  end)
end)
...
describe('table_scripts: postgres queries (built server-side)', function()
  ...
  it('CREATE To renders columns, constraints and secondary indexes server-side', function()
    local sql = action('postgres', 'CREATE To').query('public', 'users')
    -- columns: type, identity both flavors, generated, plain default, NOT NULL
    assert.is_truthy(sql:find('format_type(a.atttypid, a.atttypmod)', 1, true))
    assert.is_truthy(sql:find("a.attidentity = 'a' THEN ' GENERATED ALWAYS AS IDENTITY'", 1, true))
    assert.is_truthy(sql:find("a.attidentity = 'd' THEN ' GENERATED BY DEFAULT AS IDENTITY'", 1, true))
    assert.is_truthy(sql:find("a.attgenerated = 's'", 1, true))
    assert.is_truthy(sql:find('pg_get_expr(d.adbin, d.adrelid)', 1, true))
    -- constraints inline via the server's own renderer, PK ordered first
    assert.is_truthy(sql:find('pg_get_constraintdef(con.oid)', 1, true))
    assert.is_truthy(sql:find("ARRAY['p','u','c','f']", 1, true))
    -- secondary indexes as separate statements, constraint-backed ones excluded
    assert.is_truthy(sql:find('pg_get_indexdef(i.indexrelid)', 1, true))
    assert.is_truthy(sql:find('NOT i.indisprimary', 1, true))
    assert.is_truthy(sql:find('k.conindid = i.indexrelid', 1, true))
  end)
