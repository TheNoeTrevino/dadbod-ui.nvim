-- Oracle: introspection, table helpers, explain
--
-- No pagination (ROWNUM needs clause-level rewriting), no export (not a v1
-- export adapter). Every query is wrapped with SQL*Plus formatting lines.
...
---@private
-- Table helpers share a common FROM/qualify fragment and are each wrapped with
-- SQL*Plus COLUMN formatting, built in a loop below.
local helpers_from = table.concat({
...
---@private
local helpers_qualify_and_order_by = table.concat({
  "L.table_name = '{table}'",
  'ORDER BY',
  '\t',
}, '\n')
...
---@private
local helpers_key_cmd = table.concat({
  'SELECT',
  '\tL.table_name,',
  '\tL.column_name',
  helpers_from,
  'WHERE',
  "\tN.constraint_type = '%s'",
  '\tAND ' .. helpers_qualify_and_order_by .. 'L.column_name',
}, '\n')

---@private
local table_helpers = {
  Columns = 'DESCRIBE "{schema}"."{table}"',
  ['Foreign Keys'] = helpers_key_cmd:format('R'),
  Indexes = table.concat({
    ...
    helpers_from,
    'WHERE',
    '\t' .. helpers_qualify_and_order_by .. 'N.index_name',
  }, '\n'),
  List = 'SELECT * FROM "{schema}"."{table}"',
  ['Primary Keys'] = helpers_key_cmd:format('P'),
...
for helper, query in pairs(table_helpers) do
  table_helpers[helper] = table.concat({
    'SET linesize 4000;',
    'SET pagesize 4000;',
    '',
...
SELECT /*csv*/ O.owner, O.object_name, LOWER(O.object_type)
 ...
 ORDER BY O.owner, O.object_name]]
  ...
  explain = { plain = 'EXPLAIN PLAN FOR {sql};\nSELECT * FROM TABLE(DBMS_XPLAN.DISPLAY);' },
}
